...
class SecurityWidget {
    ...
    private injectWidget() {
        ...
        this.container.id = 'secureweb-ai-widget-root';
    ...
    private updateUI(result: AnalysisResult) {
        if (!this.scoreElement || !this.statusElement) return;

        this.scoreElement.textContent = `${result.score}%`;
        this.statusElement.textContent = result.status;

        // Update color based on status
        this.statusElement.className = 'value';
        if (result.status === 'SAFE') this.statusElement.classList.add('status-safe');
        else if (result.status === 'WARNING') this.statusElement.classList.add('status-warning');
        else if (result.status === 'DANGEROUS') this.statusElement.classList.add('status-danger');
